...
class ICLGenomicsDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.shots = shots
        self.label_to_token = (
            {0: "A", 1: "N"} if label_to_token is None else label_to_token
        )
    ...
    def get_sample_from_idx(self, idx):
        ...
        if len(self.label_to_token[y]) > 1:
            ...
            target = [
                self.tokenizer._vocab_str_to_int.get(
                    self.label_to_token[y], self.tokenizer._vocab_str_to_int["[UNK]"]
                )
            ]
        else:
            target = self.tokenizer(self.label_to_token[y], add_special_tokens=False)[
                "input_ids"
            ]
        ...
        eos_token = (
            ...
            if not exists(self.eos_token)
            else self.tokenizer(self.eos_token, add_special_tokens=False)["input_ids"]
        )
        if self.add_eos:
            seq = seq + eos_token
        if self.add_eos:
    ...
    def __getitem__(self, idx):
        ...
        for label in self.unique_labels:
            ...
            shot_indices[label] = np.random.choice(
                label_indices, size=self.shots, replace=False
            )
