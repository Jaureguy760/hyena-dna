...
class VitAttention(SequenceModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if packed_linear:
            ...
            self.q_proj = hydra.utils.instantiate(
                linear_cfg, dim, dim, bias=qkv_bias, _recursive_=False
            )
            self.k_proj = hydra.utils.instantiate(
                linear_cfg, dim, dim, bias=qkv_bias, _recursive_=False
            )
            self.v_proj = hydra.utils.instantiate(
                linear_cfg, dim, dim, bias=qkv_bias, _recursive_=False
            )
    ...
    def forward(self, x, state=None):
        ...
        if self.packed_linear:
            ...
            q, k, v = [
                rearrange(x, "b n (h d) -> b h n d", h=self.num_heads)
                for x in (q, k, v)
            ]
        ...
        attn = torch.empty(
            bsz * num_heads, q_seq_len, k_seq_len, dtype=q.dtype, device=q.device
        )
        attn = rearrange(
            torch.baddbmm(attn, q, k, beta=0, alpha=self.scale),
            "(b h) t s -> b h t s",
            h=self.num_heads,
        )
