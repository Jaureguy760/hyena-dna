...
def soft_prompting():
    ...
    parser.add_argument(
        "--config", default="./configs/evals/soft_prompting_genomics.yaml"
    )
    ...
    ds_iter = (
        ...
        else zip([args.dataset], [datasets[args.dataset]])
    )
        ...
        for shots in cfg_dataset["shots"]:
            ...
            for soft_tokens in cfg_tuning["soft_tokens"]:
                ...
                pd.concat(df_results).to_csv(
                    os.path.join(
                        args.results, f"soft_prompting_performance_{dataset}.csv"
                    )
                )
...
def load_model(
    ...
    return LitModel(
        ...
        soft_token_pdrop=soft_token_pdrop,
...
class LitModel(pl.LightningModule):
    def __init__(
        ...
        soft_tokens = (
            torch.nn.Parameter(torch.zeros(n_soft_tokens, d_model))
            if n_soft_tokens > 0
            else None
        )
        ...
        self.soft_tokens_drop = (
            torch.nn.Dropout(soft_token_pdrop)
            if soft_token_pdrop > 0
    ...
    def forward(self, x: torch.Tensor):
        ...
        if exists(self.soft_tokens):
            hidden_states = torch.cat(
                [
                    repeat(
                        self.soft_tokens_drop(self.soft_tokens),
                        "n d -> b n d",
        ...
        hidden_states = self.model.backbone.ln_f(
            residual.to(dtype=self.model.backbone.ln_f.weight.dtype)
        )
...
def tune_model(
    model, loader, cfg_tuning, verbose: bool = True, rng: np.random.RandomState = None
):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer=optimizer, mode="min", factor=0.1, patience=0
    )
    ...
    for epoch in range(cfg_tuning["max_epochs"]):
        ...
        for i, (x, y) in enumerate(loader.train_dataloader()):
            ...
            torch.nn.utils.clip_grad_norm_(
                model.parameters(), cfg_tuning.get("gradient_clip_val", 1.0)
            )
        ...
        if (epoch > 0 and sum(val_losses[-1] >= val_losses[:-1]) > 1) or (
            epoch + 1
        ) >= cfg_tuning["max_epochs"]:
            break

    best_model = best_model.to(DEVICE)
    requires_grad(
        best_model, True
    )  # we turn grads back on for completion, even though model will not be trained further...
