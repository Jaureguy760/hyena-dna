...
def instruction_tuned_ICL():
    ...
    parser.add_argument(
        "--config", default="./configs/evals/instruction_tuned_genomics.yaml"
    )
    ...
    for tuning_samples in cfg_tuning["tuning_samples"]:
        ...
        for shots in cfg_dataset["shots"]:
            ...
            for dataset, dataset_cfg in datasets.items():
                ...
                tuning_loss_mask = (
                    tuning_y_idx - 1
                )  # prediction is always from previous token
...
class LitModel(pl.LightningModule):
    ...
    def step(
        ...
        loss_mask: tp.Union[int, np.ndarray] = -1,
        phase: str = "train",
    ):
