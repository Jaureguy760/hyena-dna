...
cufftdx_lut_filename = (
    "mathdx/22.02/include/cufftdx/include/database/lut_defines_0.hpp.inc"
)
matches = set()
...
defs = [
    f"#define T_{N}_{n} {{{c:.40f},{s:.40f}}}"
    for n, c, s in zip(numerator, cos, sin)
    if n not in matches
]
def_string = "\n".join(defs)
print(def_string)
